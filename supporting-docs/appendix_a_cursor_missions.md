...
{% step %}
### Create Mission Index (MISSIONS\_QUICK\_GUIDE.md)
...
**Standard approach**: \[How features in this mission should be built]

**Error handling**: \[Mission-specific error patterns]

**Testing requirements**: \[What tests are required for this mission]
...
**Trigger**: \[When this happens] **Files to check**: \[Where to look] **Typical changes**: \[What usually needs updating]
...
**Issue**: \[Common problem] **Cause**: \[Why it happens] **Solution**: \[How to fix]
...
## Update .cursorrules to Reference Missions
...
* Project is small (<5 features) — just use a well-organized `.cursorrules` file
* Features are tightly coupled — missions create more overhead than value
* You're the only developer — less benefit if you don't need coordination across teams
* Just starting MVP — wait until project structure stabilizes before creating missions
...
Process to create a mission:

{% stepper %}
{% step %}
Add entry to `MISSIONS_QUICK_GUIDE.md`
{% endstep %}

{% step %}
Create `mission_XX_[name].md` from the template
{% endstep %}

{% step %}
Update `.cursorrules` to reference new mission
{% endstep %}

{% step %}
Update relevant mission files if there are new integration points
{% endstep %}
{% endstepper %}
...
{% stepper %}
{% step %}
Update mission spec file
{% endstep %}

{% step %}
Update `MISSIONS_QUICK_GUIDE.md` if keywords or triggers change
{% endstep %}

{% step %}
Commit with `docs(mission-XX): [description]`
{% endstep %}
{% endstepper %}
...
{% stepper %}
{% step %}
Mark as deprecated in `MISSIONS_QUICK_GUIDE.md`
{% endstep %}

{% step %}
Archive mission file to `docs/missions/archived/`
{% endstep %}
