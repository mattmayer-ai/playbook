...
### 1. Docker MCP (for local development)
...
### 2. GitHub MCP (for issue tracking and PRs)
...
2. Add to MCP config (above)

3. **Security note**: Don't commit config with token. Use environment variable:
   ```jsonc
   "env": {
...
### 3. Sequential-Thinking MCP (for complex reasoning)
...
### 4. PostgreSQL MCP (for database inspection)
...
### 5. Filesystem MCP (for external codebases)
...
**Check**:
1. Is MCP config file in correct location? (`~/.cursor/mcp.json` or `.cursor/mcp.json`)
2. Is JSON valid? (Use JSON validator)
3. Are paths correct? (`/usr/local/bin/docker` exists?)
...
This is what I use for typical product development:
